...
impl Parser {
    ...
    fn parse_token(input: &mut Peekable<Chars>) -> Result<String, ()> {
        ...
        while let Some(curr_char) = input.peek() {
            if !":/!#$%&'*+-.^_`|~".contains(curr_char.to_owned())
                && !curr_char.is_ascii_alphanumeric()
            {
                return Ok(output_string);
            }
...
mod tests {
    ...
    fn parse_token() {
        assert_eq!(
            "smth".to_owned(),
            Parser::parse_token(&mut "smth".chars().peekable()).unwrap()
        )
    }
}
