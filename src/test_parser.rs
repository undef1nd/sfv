...
fn parse_list_of_numbers() -> Result<(), Box<dyn Error>> {
    let mut input = "1,42".bytes().peekable();
...
fn parse_list_of_lists() -> Result<(), Box<dyn Error>> {
    let mut input = "(1 2), (42 43)".bytes().peekable();
    let item1 = Item::new(1.into());
...
fn parse_list_of_lists_with_param_and_spaces() -> Result<(), Box<dyn Error>> {
    let mut input = "(  1  42  ); k=*".bytes().peekable();
    let item1 = Item::new(1.into());
    let item2 = Item::new(42.into());
...
fn parse_list_errors() -> Result<(), Box<dyn Error>> {
    ...
    let mut input = "a, b c".bytes().peekable();
    assert_eq!(
        Err("parse_list: trailing characters after list member"),
        List::parse(&mut input)
    );

    let mut input = "a,".bytes().peekable();
    ...
    let mut input = "(a b),".bytes().peekable();
    assert_eq!(Err("parse_list: trailing comma"), List::parse(&mut input));

    let mut input = "(1, 2, (a b)".bytes().peekable();
    assert_eq!(
...
fn parse_inner_list_errors() -> Result<(), Box<dyn Error>> {
    let mut input = "c b); a=1".bytes().peekable();
    assert_eq!(
...
fn parse_item_int_with_space() -> Result<(), Box<dyn Error>> {
    let mut input = "12 ".bytes().peekable();
...
fn parse_item_decimal_with_bool_param_and_space() -> Result<(), Box<dyn Error>> {
    let mut input = "12.35;a ".bytes().peekable();
...
fn parse_item_number_with_param() -> Result<(), Box<dyn Error>> {
    ...
    assert_eq!(
        ...
        Item::parse(&mut "\"12.35\";a1=*".bytes().peekable())?
    );
    Ok(())
}
...
fn parse_dict_empty() -> Result<(), Box<dyn Error>> {
    assert_eq!(
        Dictionary::new(),
        Dictionary::parse(&mut "".bytes().peekable())?
    );
    Ok(())
}
...
fn parse_dict_errors() -> Result<(), Box<dyn Error>> {
    let mut input = "abc=123;a=1;b=2 def".bytes().peekable();
    assert_eq!(
        Err("parse_dict: trailing characters after dictionary member"),
        Dictionary::parse(&mut input)
    );
    let mut input = "abc=123;a=1,".bytes().peekable();
    assert_eq!(
...
fn parse_dict_with_token_param() -> Result<(), Box<dyn Error>> {
    let mut input = "a=1, b;foo=*, c=3".bytes().peekable();
    let item2_params =
...
fn parse_dict_multiple_spaces() -> Result<(), Box<dyn Error>> {
    ...
    let mut input1 = "a=1 ,  b=2".bytes().peekable();
    let mut input2 = "a=1\t,\tb=2".bytes().peekable();
    let mut input3 = "a=1, b=2".bytes().peekable();
...
fn parse_bare_item() -> Result<(), Box<dyn Error>> {
    assert_eq!(
        ...
        Parser::parse_bare_item(&mut "?0".bytes().peekable())?
    );
    assert_eq!(
        BareItem::String("test string".to_owned()),
        Parser::parse_bare_item(&mut "\"test string\"".bytes().peekable())?
    );
    assert_eq!(
        BareItem::Token("*token".to_owned()),
        Parser::parse_bare_item(&mut "*token".bytes().peekable())?
    );
    assert_eq!(
        BareItem::ByteSeq("base_64 encoding test".to_owned().into_bytes()),
        Parser::parse_bare_item(&mut ":YmFzZV82NCBlbmNvZGluZyB0ZXN0:".bytes().peekable())?
    );
    assert_eq!(
        BareItem::Decimal(Decimal::from_str("-3.55")?),
        Parser::parse_bare_item(&mut "-3.55".bytes().peekable())?
    );
    Ok(())
}
...
fn parse_bare_item_errors() -> Result<(), Box<dyn Error>> {
    ...
    assert_eq!(
        Err("parse_bare_item: item type can't be identified"),
        Parser::parse_bare_item(&mut "_11abc".bytes().peekable())
    );
    assert_eq!(
        Err("parse_bare_item: item type can't be identified"),
        Parser::parse_bare_item(&mut "   ".bytes().peekable())
    );
    Ok(())
}
...
fn parse_bool() -> Result<(), Box<dyn Error>> {
    let mut input = "?0gk".bytes().peekable();
    assert_eq!(false, Parser::parse_bool(&mut input)?);
    assert_eq!(input.collect::<Vec<_>>(), b"gk");

    assert_eq!(false, Parser::parse_bool(&mut "?0".bytes().peekable())?);
    assert_eq!(true, Parser::parse_bool(&mut "?1".bytes().peekable())?);
    Ok(())
}
...
fn parse_bool_errors() -> Result<(), Box<dyn Error>> {
    assert_eq!(
        ...
        Parser::parse_bool(&mut "".bytes().peekable())
    );
    assert_eq!(
...
fn parse_string() -> Result<(), Box<dyn Error>> {
    let mut input = "\"some string\" ;not string".bytes().peekable();
    assert_eq!("some string".to_owned(), Parser::parse_string(&mut input)?);
    assert_eq!(input.collect::<Vec<_>>(), " ;not string".as_bytes());

    assert_eq!(
        "test".to_owned(),
        Parser::parse_string(&mut "\"test\"".bytes().peekable())?
    );
    assert_eq!(
        r#"te\st"#.to_owned(),
        Parser::parse_string(&mut "\"te\\\\st\"".bytes().peekable())?
    );
    assert_eq!(
        "".to_owned(),
        Parser::parse_string(&mut "\"\"".bytes().peekable())?
    );
    assert_eq!(
        "some string".to_owned(),
        Parser::parse_string(&mut "\"some string\"".bytes().peekable())?
    );
    Ok(())
}
...
fn parse_string_errors() -> Result<(), Box<dyn Error>> {
    ...
    assert_eq!(
        ...
        Parser::parse_string(&mut "\"\\l\"".bytes().peekable())
    );
    assert_eq!(
        Err("parse_string: invalid string character"),
        Parser::parse_string(&mut "\"\u{1f}\"".bytes().peekable())
    );
    assert_eq!(
        Err("parse_string: no closing '\"'"),
        Parser::parse_string(&mut "\"smth".bytes().peekable())
    );
    Ok(())
}
...
fn parse_token() -> Result<(), Box<dyn Error>> {
    let mut input = "*some:token}not token".bytes().peekable();
    assert_eq!("*some:token".to_owned(), Parser::parse_token(&mut input)?);
    assert_eq!(input.collect::<Vec<_>>(), "}not token".as_bytes());

    assert_eq!(
        "token".to_owned(),
        Parser::parse_token(&mut "token".bytes().peekable())?
    );
    assert_eq!(
        "a_b-c.d3:f%00/*".to_owned(),
        Parser::parse_token(&mut "a_b-c.d3:f%00/*".bytes().peekable())?
    );
    assert_eq!(
        "TestToken".to_owned(),
        Parser::parse_token(&mut "TestToken".bytes().peekable())?
    );
    assert_eq!(
        "some".to_owned(),
        Parser::parse_token(&mut "some@token".bytes().peekable())?
    );
    assert_eq!(
        "*TestToken*".to_owned(),
        Parser::parse_token(&mut "*TestToken*".bytes().peekable())?
    ...
    assert_eq!(
        "test".to_owned(),
        Parser::parse_token(&mut "test token".bytes().peekable())?
    );
...
fn parse_token_errors() -> Result<(), Box<dyn Error>> {
    let mut input = "765token".bytes().peekable();
    assert_eq!(
        Err("parse_token: first character is not ALPHA or '*'"),
        Parser::parse_token(&mut input)
    );
    assert_eq!(input.collect::<Vec<_>>(), "765token".as_bytes());

    assert_eq!(
        Err("parse_token: first character is not ALPHA or '*'"),
        Parser::parse_token(&mut "7token".bytes().peekable())
    );
    assert_eq!(
        Err("parse_token: empty input string"),
        Parser::parse_token(&mut "".bytes().peekable())
    );
    Ok(())
}
...
fn parse_byte_sequence() -> Result<(), Box<dyn Error>> {
    let mut input = ":aGVsbG8:rest_of_str".bytes().peekable();
    assert_eq!(
        "hello".to_owned().into_bytes(),
        Parser::parse_byte_sequence(&mut input)?
    );
    assert_eq!("rest_of_str".as_bytes(), input.collect::<Vec<_>>());

    assert_eq!(
        "hello".to_owned().into_bytes(),
        Parser::parse_byte_sequence(&mut ":aGVsbG8:".bytes().peekable())?
    );
    assert_eq!(
        "test_encode".to_owned().into_bytes(),
        Parser::parse_byte_sequence(&mut ":dGVzdF9lbmNvZGU:".bytes().peekable())?
    );
    assert_eq!(
        "new:year tree".to_owned().into_bytes(),
        Parser::parse_byte_sequence(&mut ":bmV3OnllYXIgdHJlZQ==:".bytes().peekable())?
    );
    assert_eq!(
        "".to_owned().into_bytes(),
        Parser::parse_byte_sequence(&mut "::".bytes().peekable())?
    );
    Ok(())
}
...
fn parse_byte_sequence_errors() -> Result<(), Box<dyn Error>> {
    ...
    assert_eq!(
        ...
        Parser::parse_byte_sequence(&mut ":aGVsb G8=:".bytes().peekable())
    );
    assert_eq!(
...
fn parse_number_int() -> Result<(), Box<dyn Error>> {
    let mut input = "-733333333332d.14".bytes().peekable();
    assert_eq!(
        Num::Integer(-733333333332),
        Parser::parse_number(&mut input)?
    );
    assert_eq!("d.14".as_bytes(), input.collect::<Vec<_>>());

    assert_eq!(
        Num::Integer(42),
        Parser::parse_number(&mut "42".bytes().peekable())?
    );
    assert_eq!(
        Num::Integer(-42),
        Parser::parse_number(&mut "-42".bytes().peekable())?
    ...
    assert_eq!(
        Num::Integer(-42),
        Parser::parse_number(&mut "-042".bytes().peekable())?
    );
    assert_eq!(
        Num::Integer(0),
        Parser::parse_number(&mut "0".bytes().peekable())?
    );
    assert_eq!(
        Num::Integer(0),
        Parser::parse_number(&mut "00".bytes().peekable())?
    );
    assert_eq!(
        Num::Integer(123456789012345),
        Parser::parse_number(&mut "123456789012345".bytes().peekable())?
    );
    assert_eq!(
        Num::Integer(-123456789012345),
        Parser::parse_number(&mut "-123456789012345".bytes().peekable())?
    ...
    assert_eq!(
        Num::Integer(2),
        Parser::parse_number(&mut "2,3".bytes().peekable())?
    );
    assert_eq!(
        Num::Integer(4),
        Parser::parse_number(&mut "4-2".bytes().peekable())?
    );
    assert_eq!(
        Num::Integer(-999999999999999),
        Parser::parse_number(&mut "-999999999999999".bytes().peekable())?
    );
    assert_eq!(
        Num::Integer(999999999999999),
        Parser::parse_number(&mut "999999999999999".bytes().peekable())?
...
fn parse_number_decimal() -> Result<(), Box<dyn Error>> {
    let mut input = "00.42 test string".bytes().peekable();
    assert_eq!(
        Num::Decimal(Decimal::from_str("0.42")?),
        Parser::parse_number(&mut input)?
    );
    assert_eq!(" test string".as_bytes(), input.collect::<Vec<_>>());

    assert_eq!(
        Num::Decimal(Decimal::from_str("1.5")?),
        Parser::parse_number(&mut "1.5.4.".bytes().peekable())?
    );
    assert_eq!(
        Num::Decimal(Decimal::from_str("1.8")?),
        Parser::parse_number(&mut "1.8.".bytes().peekable())?
    );
    assert_eq!(
        Num::Decimal(Decimal::from_str("1.7")?),
        Parser::parse_number(&mut "1.7.0".bytes().peekable())?
    );
    assert_eq!(
        Num::Decimal(Decimal::from_str("3.14")?),
        Parser::parse_number(&mut "3.14".bytes().peekable())?
    );
    assert_eq!(
        Num::Decimal(Decimal::from_str("-3.14")?),
        Parser::parse_number(&mut "-3.14".bytes().peekable())?
    ...
    assert_eq!(
        ...
        Parser::parse_number(&mut "123456789012.1".bytes().peekable())?
    );
    assert_eq!(
        Num::Decimal(Decimal::from_str("1234567890.112")?),
        Parser::parse_number(&mut "1234567890.112".bytes().peekable())?
    );
...
fn parse_number_errors() -> Result<(), Box<dyn Error>> {
    let mut input = ":aGVsbG8:rest".bytes().peekable();
    assert_eq!(
        Err("parse_number: expected digit"),
        Parser::parse_number(&mut input)
    );
    assert_eq!(":aGVsbG8:rest".as_bytes(), input.collect::<Vec<_>>());

    let mut input = "-11.5555 test string".bytes().peekable();
    assert_eq!(
        Err("parse_number: too many digits after decimal point"),
        Parser::parse_number(&mut input)
    );
    assert_eq!("5 test string".as_bytes(), input.collect::<Vec<_>>());
    ...
    assert_eq!(
        ...
        Parser::parse_number(&mut "1999999999999.1".bytes().peekable())
    );
    assert_eq!(
        Err("parse_number: trailing decimal point"),
        Parser::parse_number(&mut "19888899999.".bytes().peekable())
    );
    assert_eq!(
        Err("parse_number: too many digits"),
        Parser::parse_number(&mut "1999999999999999".bytes().peekable())
    );
    assert_eq!(
        Err("parse_number: too many digits after decimal point"),
        Parser::parse_number(&mut "19999999999.99991".bytes().peekable())
    );
    assert_eq!(
        Err("parse_number: expected digit"),
        Parser::parse_number(&mut "- 42".bytes().peekable())
    ...
    assert_eq!(
        Err("parse_number: trailing decimal point"),
        Parser::parse_number(&mut "1..4".bytes().peekable())
    );
    assert_eq!(
        Err("parse_number: expected digit"),
        Parser::parse_number(&mut "-".bytes().peekable())
    );
    assert_eq!(
        Err("parse_number: trailing decimal point"),
        Parser::parse_number(&mut "-5. 14".bytes().peekable())
    );
    assert_eq!(
        Err("parse_number: trailing decimal point"),
        Parser::parse_number(&mut "7. 1".bytes().peekable())
    );
    assert_eq!(
        Err("parse_number: too many digits after decimal point"),
        Parser::parse_number(&mut "-7.3333333333".bytes().peekable())
    );
    assert_eq!(
        Err("parse_number: too many digits before decimal point"),
        Parser::parse_number(&mut "-7333333333323.12".bytes().peekable())
    );
...
fn parse_params_string() -> Result<(), Box<dyn Error>> {
    let mut input = ";b=\"param_val\"".bytes().peekable();
...
fn parse_params_bool() -> Result<(), Box<dyn Error>> {
    let mut input = ";b;a".bytes().peekable();
    let expected = Parameters::from_iter(vec![
...
fn parse_params_mixed_types() -> Result<(), Box<dyn Error>> {
    let mut input = ";key1=?0;key2=746.15".bytes().peekable();
    let expected = Parameters::from_iter(vec![
...
fn parse_params_with_spaces() -> Result<(), Box<dyn Error>> {
    let mut input = "; key1=?0; key2=11111".bytes().peekable();
    let expected = Parameters::from_iter(vec![
        ("key1".to_owned(), BareItem::Boolean(false)),
...
fn parse_params_empty() -> Result<(), Box<dyn Error>> {
    assert_eq!(
        Parameters::new(),
        Parser::parse_parameters(&mut " key1=?0; key2=11111".bytes().peekable())?
    );
    assert_eq!(
        Parameters::new(),
        Parser::parse_parameters(&mut "".bytes().peekable())?
    );
    assert_eq!(
        Parameters::new(),
        Parser::parse_parameters(&mut "[;a=1".bytes().peekable())?
    );
    assert_eq!(
        Parameters::new(),
        Parser::parse_parameters(&mut String::new().bytes().peekable())?
    );
    Ok(())
}
...
fn parse_key() -> Result<(), Box<dyn Error>> {
    assert_eq!(
        "a".to_owned(),
        Parser::parse_key(&mut "a=1".bytes().peekable())?
    );
    assert_eq!(
        "a1".to_owned(),
        Parser::parse_key(&mut "a1=10".bytes().peekable())?
    );
    assert_eq!(
        "*1".to_owned(),
        Parser::parse_key(&mut "*1=10".bytes().peekable())?
    );
    assert_eq!(
        "f".to_owned(),
        Parser::parse_key(&mut "f[f=10".bytes().peekable())?
    );
    Ok(())
}
...
fn parse_key_errors() -> Result<(), Box<dyn Error>> {
    assert_eq!(
        ...
        Parser::parse_key(&mut "[*f=10".bytes().peekable())
    );
    Ok(())
}
