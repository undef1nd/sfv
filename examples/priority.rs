...
fn parse_urgency(v: Option<sfv::Integer>) -> u8 {
    v.and_then(|v| u8::try_from(v).ok())
        .filter(|v| *v <= 7)
        .unwrap_or(Priority::default().urgency)
}

fn parse_incremental(v: Option<bool>) -> bool {
    v.unwrap_or(Priority::default().incremental)
}

impl<'input> sfv::visitor::DictionaryVisitor<'input> for Priority {
    type Error = std::convert::Infallible;

    fn entry(
        &mut self,
        key: &'input sfv::KeyRef,
    ) -> Result<impl sfv::visitor::EntryVisitor<'input>, Self::Error> {
        Ok(match key.as_str() {
...
impl<'input> sfv::visitor::ItemVisitor<'input> for PriorityParameter<'_> {
    type Error = std::convert::Infallible;

    fn bare_item(
        self,
        bare_item: sfv::BareItemFromInput<'input>,
    ) -> Result<impl sfv::visitor::ParameterVisitor<'input>, Self::Error> {
...
impl<'input> sfv::visitor::EntryVisitor<'input> for PriorityParameter<'_> {
    fn inner_list(self) -> Result<impl sfv::visitor::InnerListVisitor<'input>, Self::Error> {
        // Per https://httpwg.org/specs/rfc9218.html#parameters values of
        ...
        match self {
            Self::U(urgency) => *urgency = parse_urgency(None),
            Self::I(incremental) => *incremental = parse_incremental(None),
        }
        // Per https://httpwg.org/specs/rfc9218.html#parameters values of
